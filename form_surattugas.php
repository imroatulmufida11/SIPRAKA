...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $nomorSurat = $conn->real_escape_string($_POST['nomorSurat']);
    $namaGuru = $conn->real_escape_string($_POST['namaGuru']);
    $nipGuru = $conn->real_escape_string($_POST['nipGuru']);
    $jabatanGuru = $conn->real_escape_string($_POST['jabatanGuru']);
    $tempatPkl = $conn->real_escape_string($_POST['tempatPkl']);
    $alamatPkl = $conn->real_escape_string($_POST['alamatPkl']);
    $konsentrasiKeahlian = $conn->real_escape_string($_POST['konsentrasiKeahlian']);
    $opsi = $conn->real_escape_string($_POST['opsi']);
    $tanggalSurat = $conn->real_escape_string($_POST['tanggalSurat']);
    $siswa = $conn->real_escape_string($_POST['siswa']);

    $sql = "INSERT INTO surat_tugas (nomor_surat, nama_guru, nip_guru, jabatan_guru, tempat_pkl, alamat_pkl, konsentrasi_keahlian, opsi, tanggal_surat, siswa) 
            VALUES ('$nomorSurat', '$namaGuru', '$nipGuru', '$jabatanGuru', '$tempatPkl', '$alamatPkl', '$konsentrasiKeahlian', '$opsi', '$tanggalSurat', '$siswa')";

    if ($conn->query($sql) === TRUE) {
        $_SESSION['success'] = "Data berhasil disimpan!";
        $last_id = $conn->insert_id; // Ambil ID terakhir yang diinsert
        header("Location: surat_tugas.php?id=" . $last_id);
        exit();
    } else {
        ...
        <div class="col-12">
            <div class="bg-light rounded h-100 p-4">
                <div class="container mt-4">
                    <h2 class="text-center">Formulir Surat Tugas</h2>
                    <?php if(isset($_SESSION['success']) || isset($_SESSION['error'])): ?>
                        <div class="alert <?= isset($_SESSION['success']) ? 'alert-success' : 'alert-danger' ?>">
                    ...
                    <form method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="namaGuru" required>
                                ...
                                <?php while ($row = $resultPembimbing->fetch_assoc()) { ?>
                        ...
                        <div class="mb-3">
                            <label class="form-label">Opsi Surat</label>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" name="siswa" rows="4" placeholder="Afra-1234567890-XII/1" required></textarea>
                        </div>
